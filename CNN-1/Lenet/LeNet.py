...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        conv1_weights = tf.get_variable('weight',[10,10,c,6],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer4-conv3'):
        conv3_weights = tf.get_variable('weight',[5,5,16,16],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    #第四层：池化层，过滤器的尺寸为2×2，使用全0补充，步长为2。
    #尺寸变化：10×10×6->5×5×16
    with tf.variable_scope('layer5-pool3'):
   ...
   #将第四层池化层的输出转化为第五层全连接层的输入格式。第四层的输出为5×5×16的矩阵，然而第五层全连接层需要的输入格式
    ...
    with tf.variable_scope('layer5-fc1'):
        fc1_weights = tf.get_variable('weight',[nodes,240],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None:
            tf.add_to_collection('losses',regularizer(fc1_weights))
        fc1_biases = tf.get_variable('bias',[240],initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('layer6-fc2'):
        fc2_weights = tf.get_variable('weight',[240,120],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None:
            tf.add_to_collection('losses',regularizer(fc2_weights))
        fc2_biases = tf.get_variable('bias',[120],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer7-fc3'):
        fc3_weights = tf.get_variable('weight',[120,3],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None:
