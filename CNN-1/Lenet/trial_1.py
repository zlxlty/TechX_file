...
def module (name, input_tensor, width, height, inputdepth, outputdepth):
    with tf.variable_scope(name):
        conv1_weights = tf.get_variable('weight',[width,height,inputdepth,outputdepth],initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv1_biases = tf.get_variable('bias',[outputdepth],initializer=tf.constant_initializer(0.0))
...
def inference(input_tensor,train,regularizer):
    ...
    relu1a = module('layer-conv1a', input_tensor, 5, 5, c, 128)
    relu1b = module('layer-conv1b', input_tensor, 3, 3, c, 128)
    relu1c = module('layer-conv1c', relu1b, 3, 3, 128, 128)
    ...
    with tf.name_scope('layer2-pool1'):
        pool1a = tf.nn.max_pool(relu1a,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        pool1b = tf.nn.max_pool(relu1c,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        
    pool1 = tf.concat(axis=3, values=[pool1a, pool1b], name='pool1')

    #第三层：卷积层，过滤器的尺寸为5×5，深度为16,不使用全0补充，步长为1。
    #尺寸变化：14×14×6->10×10×16
    relu2a = module('layer-conv2a', pool1, 5, 5, 256, 64)
    relu2b = module('layer-conv2b', pool1, 3, 3, 256, 64)
    relu2c = module('layer-conv2c', relu2b, 3, 3, 64, 32)
    ...
    with tf.variable_scope('layer4-pool2'):
        pool2a = tf.nn.max_pool(relu2a,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        pool2b = tf.nn.max_pool(relu2c,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

    pool2 = tf.concat(axis=3, values=[pool2a, pool2b], name='pool2')
    ...
    with tf.variable_scope('layer7-fc3'):
        fc3_weights = tf.get_variable('weight',[84,5],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None:
            tf.add_to_collection('losses',regularizer(fc3_weights))
        fc3_biases = tf.get_variable('bias',[5],initializer=tf.truncated_normal_initializer(stddev=0.1))
